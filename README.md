...
Локальная LLM-реализация агента подключена к Ollama на localhost. При
недоступной модели агент сохраняет транскрипт и возвращает предупреждение без
выдуманных поручений. Streamlit UI и DOCX export проверены на тестовом
`MeetingResult`: исправления исполнителя, срока и имени говорящего попадают в
скачанный и повторно открытый DOCX.

Полный MP3/WAV smoke на Thunderobot с Windows пока блокирует реализация STT из
`feature/audio`: она использует MLX Whisper для Apple Silicon. UI показывает
эту проблему как ошибку стадии распознавания и не подменяет её demo-данными.
...
В режиме локальной обработки UI вызывает `app.services.audio.process_audio(audio_path)`, затем
`MeetingProtocolAgent().run(segments, meeting_date, title)`. Ошибка любого модуля показывается с
конкретной стадией, а demo-данные не подменяют результат загруженного аудио.
...
- текущий `process_audio` использует MLX Whisper для Apple Silicon и не запускается на Thunderobot с Windows без замены backend владельцем `feature/audio`;
- bundled LibreOffice отсутствует на тестовой машине, поэтому DOCX проверен структурным открытием через `python-docx`, а не пакетным визуальным рендером;
